    ...
    return (
      <>
        ...
        >
          {skus.length > 1 ? (
            <div
              className={`${styles.sizes_wrapper} ${isLoading ? styles.center : undefined
                }`}
            >
              {skus.map((sku) => (
                <SwiperSlide
            ...
            <div className={styles.single_size_wrapper}>
              {skus.map((singleSku) => (
                <SwiperSlide
                  ...
                  className={`${singleSku.quantity > 0
                    ? styles.single_size
                    : styles.single_size_no_quantity
                    } ${isLoading && styles.no_show} ${isSmallContainer ? styles.is_small_container : undefined
                    }`}
                >
                  Add To Bag
                </SwiperSlide>
        ...
        {isLoading && (
          <div
            className={`${styles.loader} ${isSmallContainer ? styles.is_small_container : undefined
              }`}
          />
        )}
      </>
